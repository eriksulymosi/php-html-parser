...
class Cleaner implements CleanerInterface
{
    ...
    public function clean(string $str, Options $options, string $defaultCharset): string
    {
        ...
        $str = \mb_eregi_replace('"\s+>', '">', (string) $str);
        if ($str === false) {
        ...
        if ($options->isRemoveStyles()) {
            $str = \mb_eregi_replace("<\s*style[^>]*[^/]>(.*?)<\s*/\s*style\s*>", '', (string) $str);
            if ($str === false) {
            ...
            $str = \mb_eregi_replace("<\s*style\s*>(.*?)<\s*/\s*style\s*>", '', (string) $str);
            if ($str === false) {
        ...
        if ($options->isRemoveSmartyScripts()) {
            $str = \mb_eregi_replace("(\{\w)(.*?)(\})", '', (string) $str);
            if ($str === false) {
     ...
     * @throws LogicalException
     */
    private function setUpRegexEncoding(string $str, Options $options, string $encoding): string
    {
