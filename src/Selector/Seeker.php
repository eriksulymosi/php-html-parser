...
class Seeker implements SeekerInterface
{
    ...
    public function seek(array $nodes, RuleDTO $rule, array $options): array
    {
        ...
        foreach ($nodes as $node) {
            ...
            while (!\is_null($child)) {
                ...
                if ($child instanceof InnerNode && $child->hasChildren() && (!isset($options['checkGrandChildren'])
    ...
    {
        if ($rule->getTag() !== '' && $rule->getTag() !== '0' && $rule->getTag() !== \mb_strtolower($node->getTag()->name())
            && $rule->getTag() !== '*'
        ) {
            return false;
        }
